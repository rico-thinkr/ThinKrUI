...
local bar = CreateFrame("Frame", "ThinKrUIMenubar", UIParent)
...
ThinKrUIStatsButton = addButton("", POSITION_MIDDLE, function()
	TimeManagerClockButton_OnClick(TimeManagerClockButton)
end)

ThinKrUIStatsButton:SetWidth(200)
...
addButton("Toggle DBM", POSITION_RIGHT, function()
	if IsAddOnLoaded("DBM-Core") then
		...
		DEFAULT_CHAT_FRAME:AddMessage("ThinKrUI: |cffffffffDBM disabled. Type|r /rl |cfffffffffor the changes to apply.|r", r, g, b)
	else
		EnableAddOn("DBM-Core")
		EnableAddOn("DBM-StatusBarTimers")
		DEFAULT_CHAT_FRAME:AddMessage("ThinKrUI: |cffffffffDBM enabled. Type|r /rl |cfffffffffor the changes to apply.|r", r, g, b)
	end
end)
...
--[[addButton("Toggle damage meter", POSITION_RIGHT, function()
	if IsAddOnLoaded("alDamageMeter") then
		...
		DEFAULT_CHAT_FRAME:AddMessage("ThinKrUI: |cffffffffalDamageMeter disabled. Type|r /rl |cfffffffffor the changes to apply.|r", r, g, b)
	else
		EnableAddOn("alDamageMeter")
		LoadAddOn("alDamageMeter")
		if IsAddOnLoaded("alDamageMeter") then
			DEFAULT_CHAT_FRAME:AddMessage("ThinKrUI: |cffffffffalDamageMeter loaded.|r", r, g, b)
		else
			DEFAULT_CHAT_FRAME:AddMessage("ThinKrUI: |cffffffffalDamageMeter not found!|r", r, g, b)
		end
	end
end)]]
