...
addonLoaded = function(_, addon)
	...
	if ThinKrUIConfig.layout == nil then ThinKrUIConfig.layout = 1 end
...
local PostUpdateHealth = function(Health, unit, min, max)
	...
	if ThinKrUIConfig.layout == 2 and not C.unitframes.healerClasscolours then
...
if IsAddOnLoaded("Blizzard_CompactRaidFrames") then
	CompactRaidFrameManager:SetParent(ThinKrUIHider)
...
for i = 1, MAX_PARTY_MEMBERS do
	...
	_G[pet]:SetParent(ThinKrUIHider)
...
local Shared = function(self, unit, isSingle)
	...
	if ThinKrUIConfig.layout == 1 or C.unitframes.healerClasscolours then
	...
	if ThinKrUIConfig.layout == 2 then
...
local UnitSpecific = {
	...
	player = function(self, ...)
		...
		if C.actionbars.enable then
			ThinKrUI_LeaveVehicle:SetPoint("RIGHT", self, "LEFT", -9, 0)
		end
	end,
...
oUF:Factory(function(self)
	...
	if ThinKrUIConfig.layout == 1 then
		if C.unitframes.autoPosition then
	...
	local party = self:SpawnHeader(nil, nil, "party,raid",
		'showParty', true,
		'showPlayer', ThinKrUIConfig.layout == 2,
		'showSolo', false,
		'yoffset', -3,
		'maxColumns', 5,
		'unitsperColumn', 1,
